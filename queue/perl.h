...
#define binit(I,M) bul queue ## M
#define bstop(I,M) U_ queueDestroy(HQ_ n)
#define jni(A,B) A ## B
#define bq(I,M,E) Z_ jni(queue,M)(C(HQ_)_j##I){rt(_j##I);say(E);}
#define be(I,M,P) bul jni(queue,M)(C(HQ_)_e##I){rt(_e##I);say P(Sv(YR)==0);}
#define BEGIN(N,P) bul jni(queue,N)(HQ_ _p1, P UR_ p0)
#define END(N,P) UR_ jni(queue,N)(C(HQ_) _p2)
#define ni ++i
#define unless(P) if(!(P))

#define ms Size
#define mc Capacity
#define me ElementSize
#define mie IsEmpty
#define mne IsFull
#define l0 Enqueue
#define l1 Dequeue
